...
async function drawLineChart() {
  ...
  const xAccessor = d => dateParser(d.date); // 2. Create chart dimensions
  ...
  const bounds = wrapper.append('g').style('transform', "translate(".concat(dimensions.margin.left, "px, ").concat(dimensions.margin.top, "px)")); // 4. Create scales
  ...
  const xScale = d3.scaleTime().domain(d3.extent(dataset, xAccessor)).range([0, dimensions.boundedWidth]); // 5. Draw data

  const lineGenerator = d3.line().x(d => xScale(xAccessor(d))).y(d => yScale(yAccessor(d)));
  const line = bounds.append('path').attr('d', lineGenerator(dataset)).attr('fill', 'none').attr('stroke', '#af9358').attr('stroke-width', 2); // 6. Draw peripherals
